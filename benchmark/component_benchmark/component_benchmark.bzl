load("@npm_dev_infra//benchmark/ng_rollup_bundle:ng_rollup_bundle.bzl", "ng_rollup_bundle")
...
def copy_default_file(origin, destination):
    ...
    native.genrule(
        ...
        srcs = ["@npm_dev_infra//benchmark/component_benchmark/defaults:" + origin],
        outs = [destination],
        cmd = "cat $(SRCS) >> $@",
    )
...
def component_benchmark(
    ...
    ** USAGE NOTES **
    ...
    ng_module(
        ...
        deps = ng_deps,
        tsconfig = "@npm_dev_infra//benchmark/component_benchmark:tsconfig-e2e.json",
    )
    ...
    ts_devserver(
        ...
        additional_root_paths = ["@npm_dev_infra//benchmark/component_benchmark/defaults"],
